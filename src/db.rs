...
async fn init_pool(db_config: &DatabaseConfig) -> Result<PgPool, sqlx::Error> {
    // Build connection URL with timeout parameter
    let url_with_timeout = if db_config.url.contains('?') {
        format!("{}&connect_timeout={}", db_config.url, db_config.connection_timeout)
    } else {
        format!("{}?connect_timeout={}", db_config.url, db_config.connection_timeout)
    };
